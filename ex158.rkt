...
(check-expect (tock '()) '())
(check-expect (tock (cons -1 '())) (cons -2 '()))
(check-expect (tock (cons (- (/ (image-height SHOT) 2)) '())) '())
...
(define (tock w)
  (cond
    [(empty? w) '()]
    [else (if (visible? (first w))
              (cons (sub1 (first w)) (tock (rest w)))
              (tock (rest w)))]))

; Number -> Boolean
; determines if the shot is visible

(check-expect (visible? -64) #false)
(check-expect (visible? -1) #true)
(check-expect (visible? 0) #true)
(check-expect (visible? 42) #true)

(define (visible? s)
  (> s (- (/ (image-height SHOT) 2))))
