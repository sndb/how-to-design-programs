...
;   (make-game UFO Tank [List-of Missile])
...
(define ex-g (make-game ex-u ex-t `(,ex-m)))
...
(check-expect (missiles-render '() BACKGROUND)
              BACKGROUND)

(check-expect
 (missiles-render `(,(make-posn 22 103))
                  BACKGROUND)
 (place-image MISSILE 22 103 BACKGROUND))
...
(check-expect (missiles-move '())
              '())
(check-expect (missiles-move `(,(make-posn 100 120)))
              `(,(make-posn 100 (- 120 MISSILE-SPEED))))
(check-expect (missiles-move `(,(make-posn 100 -20)))
              '())
...
(define (missiles-move l)
  (local (; Missile -> Missile
          ...
          (define (missile-visible? m)
            (> (posn-y m) 0)))
    (filter missile-visible? (map missile-move l))))
