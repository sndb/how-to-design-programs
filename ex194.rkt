...
(define (render-poly img p)
  (render-line (connect-dots img (remove-last p) (last p))
               (first p)
               (last p)))
 
; Image NELoP Posn -> Image
; connects the Posns in p and ap in an image

(check-expect (connect-dots MT
...
(define (connect-dots img p ap)
  (cond
    [(empty? (rest p)) (render-line img (first p) ap)]
...
(check-expect (remove-last triangle-p)
              (list
               (make-posn 20 10)
               (make-posn 20 20)))
