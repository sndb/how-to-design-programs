...
(define (select-album-date a d l)
  (select-date d (select-album a l)))

; Date LTracks -> LTracks
; extracts from l the list of tracks that have been played after the date d

(check-expect (select-date (create-date 2011 05 17 17 35 00) '()) '())
(check-expect (select-date (create-date 2011 05 17 17 35 00) ex-ltracks2)
              (list ex-track1 ex-track3))

(define (select-date d l)
  (cond
    [(empty? l) '()]
    [(cons? l)
     (if (date>=? (track-played (first l)) d)
         (cons (first l) (select-date d (rest l)))
         (select-date d (rest l)))]))
                             ...
                             (or (> (date-hour d1) (date-hour d2))
                                 (and (= (date-hour d1) (date-hour d2))
                                      (or (> (date-minute d1) (date-minute d2))
                                          (and (= (date-minute d1) (date-minute d2))
...
(check-expect (select-albums ex-ltracks1)
              (list (list ex-track1) (list ex-track2)))
(check-expect (select-albums ex-ltracks2)
              (list (list ex-track1 ex-track3) (list ex-track2)))

(define (select-albums l)
  (select-albums-by-titles (select-album-titles/unique l) l))

; List-of-strings LTracks -> LLTracks
; produces a list of LTracks, one per album title in t

(check-expect (select-albums-by-titles '() ex-ltracks1) '())
(check-expect
 (select-albums-by-titles (list "A Day Without Rain") ex-ltracks1)
 (list (list ex-track1)))
(check-expect
 (select-albums-by-titles (list "A Day Without Rain") ex-ltracks2)
 (list (list ex-track1 ex-track3)))
(check-expect
 (select-albums-by-titles (list "A Day Without Rain"
                                "Fumbling Towards Ecstasy")
                          ex-ltracks2)
 (list (list ex-track1 ex-track3) (list ex-track2)))

(define (select-albums-by-titles t l)
  (cond
    [(empty? t) '()]
    [(cons? t)
     (cons (select-album (first t) l)
           (select-albums-by-titles (rest t) l))]))