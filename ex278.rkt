...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname ex278) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
(define (move-worm w)
  (local (; Tail Direction -> Tail
          ; moves t in the direction d
          (define (move-tail t d)
            (reverse (cons (move-segment (head t) d)
                           (reverse (rest t))))))
...
(define (eat-food w f)
  (make-worm (reverse (cons f (reverse (worm-tail w))))
             (worm-direction w)))
  ...
  (local (; Segment Image -> Image
          ...
          (define (add-segment-to-image s img)
            (place-image SEGMENT
                         ...
                         (sg->px (posn-y s))
                         img)))
    (foldr add-segment-to-image img t)))
...
(define (final-scene g)
  (overlay/align
   "left" "bottom"
   (text (cond [(run-into-walls? (game-worm g))
                "the worm hit the wall"]
               [(run-into-itself? (game-worm g))
                "the worm ran into itself"])
         SEGMENT-SIZE "black")
   (render-game g)))
