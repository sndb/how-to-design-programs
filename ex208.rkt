;; The first three lines of this file were inserted by DrRacket. They record metadata
;; about the language level of this file in a form that our tools can easily process.
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname ex208) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/batch-io)
(require 2htdp/itunes)

(define ITUNES-LOCATION "itunes.xml")

; LLists
(define list-tracks (read-itunes-as-lists ITUNES-LOCATION))

; LAssoc Examples
(define ex-lassoc1
  (list
   (list "Track ID" 442)
   (list "Name" "Wild Child")
   (list "Artist" "Enya")
   (list "Album" "A Day Without Rain")
   (list "Genre" "New Age")
   (list "Kind" "MPEG audio file")
   (list "Size" 4562044)
   (list "Total Time" 227996)
   (list "Track Number" 2)
   (list "Track Count" 11)
   (list "Year" 2000)
   (list "Date Modified" (create-date 2002 7 17 0 0 11))
   (list "Date Added" (create-date 2002 7 17 3 55 14))
   (list "Bit Rate" 160)
   (list "Sample Rate" 44100)
   (list "Play Count" 20)
   (list "Play Date" 3388484113)
   (list "Play Date UTC" (create-date 2011 5 17 17 35 13))
   (list "Sort Album" "Day Without Rain")
   (list "Persistent ID" "EBBE9171392FA348")
   (list "Track Type" "File")
   (list
    "Location"
    "file://localhost/Users/matthias/Music/iTunes/iTunes%20Music/Enya/A%20Day%20Without%20Rain/02%20Wild%20Child.mp3")
   (list "File Folder Count" 4)
   (list "Library Folder Count" 1)))

(define ex-lassoc2
  (list
   (list "Track ID" 444)
   (list "Name" "Only Time")
   (list "Artist" "Enya")
   (list "Album" "A Day Without Rain")
   (list "Genre" "New Age")
   (list "Kind" "MPEG audio file")
   (list "Size" 4364035)
   (list "Total Time" 218096)
   (list "Track Number" 3)
   (list "Track Count" 11)
   (list "Year" 2000)
   (list "Date Modified" (create-date 2002 7 17 0 0 21))
   (list "Date Added" (create-date 2002 7 17 3 55 42))
   (list "Bit Rate" 160)
   (list "Sample Rate" 44100)
   (list "Play Count" 18)
   (list "Play Date" 3388484327)
   (list "Play Date UTC" (create-date 2011 5 17 17 38 47))
   (list "Sort Album" "Day Without Rain")
   (list "Persistent ID" "EBBE9171392FA34A")
   (list "Track Type" "File")
   (list
    "Location"
    "file://localhost/Users/matthias/Music/iTunes/iTunes%20Music/Enya/A%20Day%20Without%20Rain/03%20Only%20Time.mp3")
   (list "File Folder Count" 4)
   (list "Library Folder Count" 1)))

(define ex-lassoc3
  (list
   (list "Track ID" 486)
   (list "Name" "Possession")
   (list "Artist" "Sarah McLachlan")
   (list "Album" "Fumbling Towards Ecstasy")
   (list "Genre" "Rock")
   (list "Kind" "MPEG audio file")
   (list "Size" 5599708)
   (list "Total Time" 279875)
   (list "Track Number" 1)
   (list "Track Count" 12)
   (list "Year" 1993)
   (list "Date Modified" (create-date 2002 7 17 23 46 50))
   (list "Date Added" (create-date 2002 7 18 3 46 4))
   (list "Bit Rate" 160)
   (list "Sample Rate" 44100)
   (list "Play Count" 3)
   (list "Play Date" 3278077468)
   (list "Play Date UTC" (create-date 2007 11 16 22 4 28))
   (list "Persistent ID" "EBBE9171392FA374")
   (list "Track Type" "File")
   (list
    "Location"
    "file://localhost/Users/matthias/Music/iTunes/iTunes%20Music/Sarah%20McLachlan/Fumbling%20Towards%20Ecstasy/01%20Possession.mp3")
   (list "File Folder Count" 4)
   (list "Library Folder Count" 1)))

(define ex-lassoc4
  (list
   (list "Track ID" 2918)
   (list "Name" "Kusursuzsun (Acoustic Version)")
   (list "Artist" "Emrah")
   (list "Composer" "Various")
   (list "Album" "Putumayo Presents Turkish Groove")
   (list "Genre" "World")
   (list "Kind" "WAV audio file")
   (list "Size" 42324284)
   (list "Total Time" 239933)
   (list "Disc Number" 1)
   (list "Disc Count" 1)
   (list "Track Number" 7)
   (list "Track Count" 11)
   (list "Year" 2004)
   (list "Date Modified" (create-date 2008 11 10 16 43 50))
   (list "Date Added" (create-date 2008 11 10 16 43 36))
   (list "Bit Rate" 1411)
   (list "Sample Rate" 44100)
   (list "Play Count" 3)
   (list "Play Date" 3404827335)
   (list "Play Date UTC" (create-date 2011 11 22 22 22 15))
   (list "Compilation" #true)
   (list "Persistent ID" "1502D7F0030A1257")
   (list "Track Type" "File")
   (list
    "Location"
    "file://localhost/Users/matthias/Music/iTunes/iTunes%20Music/Compilations/Putumayo%20Presents%20Turkish%20Groove/07%20Kusursuzsun%20(Acoustic%20Version).wav")
   (list "File Folder Count" 4)
   (list "Library Folder Count" 1)))

(define ex-lassoc5
  (list
   (list "Track ID" 3978)
   (list "Name" "Tears Dry On Their Own")
   (list "Artist" "Amy Winehouse")
   (list "Album Artist" "Amy Winehouse")
   (list "Composer" "Amy Winehouse, Nickolas Ashford 38 Valerie Simpson")
   (list "Album" "iTunes Festival: London 2007")
   (list "Genre" "Pop")
   (list "Kind" "Purchased AAC audio file")
   (list "Size" 7149477)
   (list "Total Time" 200440)
   (list "Disc Number" 1)
   (list "Disc Count" 1)
   (list "Track Number" 1)
   (list "Track Count" 8)
   (list "Year" 2007)
   (list "Date Modified" (create-date 2014 10 11 23 40 1))
   (list "Date Added" (create-date 2014 10 20 14 31 25))
   (list "Bit Rate" 256)
   (list "Sample Rate" 44100)
   (list "Play Count" 6)
   (list "Play Date" 3498739159)
   (list "Play Date UTC" (create-date 2014 11 13 20 59 19))
   (list "Release Date" (create-date 2007 8 13 7 0 0))
   (list "Normalization" 7219)
   (list "Artwork Count" 1)
   (list "Persistent ID" "BBAA5285EF3D3F9F")
   (list "Track Type" "File")
   (list "Purchased" #true)
   (list
    "Location"
    "file://localhost/Users/matthias/Music/iTunes/iTunes%20Media/Music/Amy%20Winehouse/iTunes%20Festival_%20London%202007/01%20Tears%20Dry%20On%20Their%20Own.m4a")
   (list "File Folder Count" 5)
   (list "Library Folder Count" 1)))

; LLists Examples
(define ex-llists1 (list ex-lassoc1 ex-lassoc2))
(define ex-llists2 (list ex-lassoc1 ex-lassoc2 ex-lassoc3))
(define ex-llists3 (append ex-llists2 (list ex-lassoc4 ex-lassoc5)))

; String LAssoc Any -> Association
; finds the first Association whose first item is equal to key, else default

(check-expect (find-association "Track ID" '() #false) #false)
(check-expect (find-association "Track ID" ex-lassoc1 #false) (list "Track ID" 442))
(check-expect (find-association "Name" ex-lassoc2 #false) (list "Name" "Only Time"))

(define (find-association key l default)
  (cond
    [(empty? l) default]
    [(cons? l)
     (if (association-first=? (first l) key)
         (first l)
         (find-association key (rest l) default))]))

; Association String -> Boolean
; is the first item of a equal to s?

(check-expect (association-first=? (list "Name" "Wild Child") "Track ID") #false)
(check-expect (association-first=? (list "Track ID" 442) "Track ID") #true)

(define (association-first=? a s)
  (string=? (first a) s))

; LLists -> N
; produces the total amount of play time

(check-expect (total-time/list '()) 0)
(check-expect (total-time/list ex-llists1)
              (+ (second (assoc "Total Time" ex-lassoc1))
                 (second (assoc "Total Time" ex-lassoc2))))
(check-expect (total-time/list ex-llists2)
              (+ (second (assoc "Total Time" ex-lassoc1))
                 (second (assoc "Total Time" ex-lassoc2))
                 (second (assoc "Total Time" ex-lassoc3))))

(define (total-time/list l)
  (cond
    [(empty? l) 0]
    [(cons? l)
     (+ (second (find-association "Total Time" (first l) 0))
        (total-time/list (rest l)))]))

; LLists -> List-of-strings
; produces the Strings that are associated with a Boolean attribute

(check-expect (boolean-attributes '()) '())
(check-expect (boolean-attributes ex-llists2) '())
(check-expect (boolean-attributes ex-llists3)
              (list "Compilation" "Purchased"))

(define (boolean-attributes l)
  (cond
    [(empty? l) '()]
    [(cons? l)
     (create-set
      (if (not (empty? (boolean-associations (first l))))
          (append (association-strings (boolean-associations (first l)))
                  (boolean-attributes (rest l)))
          (boolean-attributes (rest l))))]))

; List-of-strings -> List-of-strings
; constructs a list that contains every String from l exactly once

(check-expect (create-set '()) '())
(check-expect (create-set (list "A Day Without Rain" "Fumbling Towards Ecstasy"))
              (list "A Day Without Rain" "Fumbling Towards Ecstasy"))
(check-expect (create-set (list "A Day Without Rain" "Fumbling Towards Ecstasy" "A Day Without Rain"))
              (list "A Day Without Rain" "Fumbling Towards Ecstasy"))

(define (create-set l)
  (cond
    [(empty? l) '()]
    [(cons? l)
     (cons (first l) (remove-all (first l) (create-set (rest l))))]))

; LAssoc -> List-of-strings
; produces the first items of associations l

(check-expect (association-strings '()) '())
(check-expect (association-strings ex-lassoc1)
              (list
               "Track ID"
               "Name"
               "Artist"
               "Album"
               "Genre"
               "Kind"
               "Size"
               "Total Time"
               "Track Number"
               "Track Count"
               "Year"
               "Date Modified"
               "Date Added"
               "Bit Rate"
               "Sample Rate"
               "Play Count"
               "Play Date"
               "Play Date UTC"
               "Sort Album"
               "Persistent ID"
               "Track Type"
               "Location"
               "File Folder Count"
               "Library Folder Count"))

(define (association-strings l)
  (cond
    [(empty? l) '()]
    [(cons? l)
     (cons (first (first l)) (association-strings (rest l)))]))

; LAssoc -> LAssoc
; produces the association whose second items are booleans

(check-expect (boolean-associations ex-lassoc1) '())
(check-expect (boolean-associations ex-lassoc4)
              (list (list "Compilation" #true)))
(check-expect (boolean-associations ex-lassoc5)
              (list (list "Purchased" #true)))

(define (boolean-associations l)
  (cond
    [(empty? l) '()]
    [(cons? l)
     (if (boolean-bsdn? (first l))
         (cons (first l) (boolean-associations (rest l)))
         (boolean-associations (rest l)))]))

; Association -> Boolean
; is the second item of a boolean?
(define (boolean-bsdn? a)
  (boolean? (second a)))