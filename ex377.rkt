...
; XEnum.v2 -> XEnum.v2
; replaces all "hello"s with "bye" in e

(check-expect (replace-hello-enum `(ul (li ,w0)
                                       (li ,w2)))
              `(ul (li ,w0)
                   (li ,w2)))
(check-expect (replace-hello-enum `(ul (li ,w0)
...
(define (replace-hello-enum e)
  (cons 'ul (map replace-hello-item (xexpr-content e))))

; XItem.v2 -> XItem.v2
; replaces all "hello"s with "bye" in i

(check-expect (replace-hello-item `(li ,w0)) `(li ,w0))
(check-expect (replace-hello-item `(li ,w1)) `(li ,bye))

(define (replace-hello-item i)
